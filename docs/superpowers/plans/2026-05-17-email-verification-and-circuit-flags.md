...
> **Task 2 review follow-up (MUST do here):** harden `start_trial()` before wiring it to verify-email. Its guard currently uses `select(Subscription).where(user_id==user.id).scalar_one_or_none()` — change to `.first()` (or `select(...).limit(1)` / an `exists()`), because a verified user may legitimately have ≥1 subscription row (e.g. an `event` pass bought pre-verification) and `scalar_one_or_none()` raises `MultipleResultsFound`. Keep the semantics "user already has any subscription → skip the free trial" (a user with a paid/event sub doesn't need the free trial). Add a unit test: `start_trial` is a safe no-op when the user already has 2 subscription rows (must NOT raise).
