...
- Per competitor: `retired` = **all** of its rows flagged retired (`EventType.STATUS == "sr"`) — `any` would wrongly DNF an endurance team whose reconnect left a stale `sr` on the old row while it kept racing on a new row; `laps_completed` = sum of valid laps across its row(s); `race_time_ms` = sum of those lap_ms. (Relative proxy: with a common start, fewer-time-for-equal-laps = ahead; for endurance, more laps dominates and time breaks ties — correct for "who is classified ahead".)
