...
function AddCardForm({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (submitError) {
      setError(submitError.message || t("payment.errorSaving"));
      setLoading(false);
    } else {
      onSuccess();
    }
...
export function PaymentMethodsPanel() {
  ...
  const handleDelete = async (pm: PaymentMethod) => {
    const ok = await confirm({
      title: t("payment.confirmDelete.title"),
      message: t("payment.confirmDelete.message", {
    ...
    <div className="space-y-4">
      ...
      {methods.length === 0 && !showAddForm ? (
        <div className="bg-surface border border-border rounded-xl p-6 text-center">
          ...
          <p className="text-neutral-400 text-sm">{t("payment.empty")}</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {!pm.is_default && (
                  ...
                  >
                    {actionLoading === pm.id ? "..." : t("payment.default")}
                  </button>
                )}
                <button
      ...
      {showAddForm && clientSecret ? (
        <div className="bg-surface border border-border rounded-xl p-5">
          <h4 className="text-white text-sm font-semibold mb-4">{t("payment.addCard")}</h4>
          <Elements
