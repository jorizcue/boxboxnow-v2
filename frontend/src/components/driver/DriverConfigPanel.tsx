  ...
  return (
    <div className="bg-neutral-900/98 border-b border-border/40 px-3 py-3 space-y-3 text-xs max-h-[70vh] overflow-y-auto">
      <div className="flex items-center justify-between">
        <span className="font-bold text-accent uppercase tracking-wider text-[10px]">{t("driverConfig.panel.title")}</span>
      ...
      {presets.length > 0 && (
        <div className="space-y-1">
          <label className="text-[10px] text-neutral-400 uppercase tracking-wider">{t("driverConfig.panel.template")}</label>
          <select
            value=""
          ...
          >
            <option value="">{t("driverConfig.panel.applyTemplate")}</option>
            {presets.map((p) => (
      ...
      {/* Circuit selector */}
      <div className="space-y-1">
        <label className="text-[10px] text-neutral-400 uppercase tracking-wider">{t("driverConfig.panel.circuit")}</label>
        <select
          value={config.selectedCircuitId ?? ""}
          onChange={(e) => config.setCircuitId(e.target.value ? Number(e.target.value) : null)}
          className="w-full bg-black border border-border rounded-md px-2 py-1.5 text-xs text-white"
        >
          <option value="">{t("driverConfig.panel.circuitAuto")}</option>
          {circuits.map((c) => (
      ...
      {/* Kart number */}
      <div className="space-y-1">
        <label className="text-[10px] text-neutral-400 uppercase tracking-wider">{t("driverConfig.panel.kartNumber")}</label>
        <input
          type="number"
          min={0}
          max={999}
          ...
          placeholder={t("driverConfig.panel.kartNumberPlaceholder")}
          className="w-full bg-black border border-border rounded-md px-2 py-1.5 text-xs text-white"
        />
      </div>
      ...
      <div className="space-y-2">
        <label className="text-[10px] text-neutral-400 uppercase tracking-wider">{t("driverConfig.panel.visibleCards")}</label>
