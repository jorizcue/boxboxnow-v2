 ...
 *                            modal opcional de detalle de kart).
...
/**
 * Card vertical con la puntuación de box (0–100) + grid de FIFO con
 * todas las líneas configuradas. Acepta clic en cada entrada para
 * abrir el modal de detalle (igual que en la pestaña Box).
 *
 * Renombrar/duplicar el contenido equivalente del componente FifoQueue
 * romperia la paridad visual entre ambas pestañas — el componente vive
 * aquí y FifoQueue lo importa.
 */
export function BoxFifoView() {
  ...
  // historia (placeholders de arranque). Mismo cálculo que en
  // FifoQueue — se necesita aquí porque el render del grid lo
  // consume entry-a-entry.
  ...
  // Mismo split en filas que FifoQueue: defaults a la izquierda,
  // pit-ins reales a la derecha ordenados por línea = idx % boxLines.
  // No leemos `entry.line` porque queda obsoleto si el strategist
  // cambia boxLines mid-race (ver comentario largo en FifoQueue).
  const rows = useMemo(() => {
...
export function PitInProgressCard({ className }: { className?: string }) {
  ...
  // Mismo cálculo que pitElapsedSec en FifoQueue.tsx: tomamos
  // `pit_history[-1].raceTimeMs` y restamos del race-time-elapsed
  // actual. Si no hay registro, devolvemos 0. El cálculo coincide
  // con el del backend (race_time_ms = duration_min*60*1000 - countdown).
